...
function getTravelRecommendation(e) {
  const ubikeStation =  e.target.parentElement.parentElement;
  const stationTitle = ubikeStation.querySelector(".station-name").innerText.split("：")[1];
  const stationLocation = ubikeStation.querySelector(".station-location").innerText.split("：")[1];
  const travelInfoContent = document.querySelector("#travel-recommendation-info");
